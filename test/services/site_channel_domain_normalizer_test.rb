...
class SiteChannelDomainNormalizerTest < ActiveJob::TestCase
  test "when offline normalizes the domain" do
    ...
    assert_equal "example2.com", SiteChannelDomainNormalizer.new(domain: "example2.com").perform
  ...
  test "raises exception with invalid url with protocol" do
    assert_raises(URI::InvalidURIError) do
      SiteChannelDomainNormalizer.new(domain: "https://bad url.com").perform
    end
  end

  test "raises exception with invalid url without protocol" do
    assert_raises(URI::InvalidURIError) do
      SiteChannelDomainNormalizer.new(domain: "bad url.com").perform
    end
  end
  ...
  test "when online handles normalization failures by raising DomainExclusionError" do
    ...
    assert_raises(SiteChannelDomainNormalizer::DomainExclusionError) do
      SiteChannelDomainNormalizer.new(domain: "https://example3.com").perform
    end
